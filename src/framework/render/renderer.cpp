...
Renderer::~Renderer() { this->Shutdown(); }
...
int Renderer::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float aspectw, float aspecth)
{
    ...
    if (!numModes && !noSurfaceOccured)
    {
        THROW_EX_DETAILS(InitializeRendererException, "No display mode list");
    }

    UniqueRef<DXGI_MODE_DESC> displayModeList = UniqueRef<DXGI_MODE_DESC>(new DXGI_MODE_DESC[numModes]);
    ...
    {
        const auto dm = displayModeList.get();
        for (i = 0; i < numModes; i++)
        {
            Log::Logger().Info("%d %d x %d, %d, scale %d", i, dm[i].Width, dm[i].Height, dm[i].RefreshRate, dm[i].Scaling);
            if (dm[i].Width == (unsigned int)m_screenW)
            {
                if (dm[i].Height == (unsigned int)m_screenH)
                {
                    numerator = dm[i].RefreshRate.Numerator;
                    denominator = dm[i].RefreshRate.Denominator;
                }
            }
        }
    }
    ...
    result = m_device->CreateRenderTargetView(backBufferPtr.Get(), nullptr, &m_myRenderTargetView);
    if (FAILED(result))
    {
        THROW_EX_HRESULT(InitializeRendererException, result);
    }
    ...
    result = m_device->CreateRasterizerState(&rastDesc, m_rasterState.GetAddressOf());
    if (FAILED(result))
        THROW_EX_HRESULT(InitializeRendererException, result);
...
std::string Renderer::GetVideoCardInfo() const { return std::string(m_videoCardDescription); }
...
void Renderer::Resize(int screenW, int screenH)
{
    ...
    if (m_swapChain)
    {
        ...
        // Resize RTV
        hr = m_swapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(pBuffer.GetAddressOf()));

        if (FAILED(hr))
            THROW_EX_HRESULT(ResizeRenderSurfaceException, hr);

        hr = m_device->CreateRenderTargetView(pBuffer.Get(), nullptr, &m_myRenderTargetView);
...
void Renderer::BeginScene()
{
    // Clear the back buffer.
    for (size_t i = 0; i < this->m_renderTargetViewCount; i++)
        if (m_renderTargetViews[i]) { m_deviceContext->ClearRenderTargetView(m_renderTargetViews[i], black); }
    // Clear the depth buffer.
    m_deviceContext->ClearDepthStencilView(m_depthStencilView.Get(), D3D11_CLEAR_DEPTH, 1.0f, 0);
}
...
void Renderer::BeginSceneDev()
{
    // Clear the back buffer.
    for (size_t i = 0; i < this->m_renderTargetViewCount; i++)
        if (m_renderTargetViews[i]) { m_deviceContext->ClearRenderTargetView(m_renderTargetViews[i], cornflowerBlue); }
    // Clear the depth buffer.
    m_deviceContext->ClearDepthStencilView(m_depthStencilView.Get(), D3D11_CLEAR_DEPTH, 1.0f, 0);
}
...
void Renderer::SetRenderTargetView(const ComPtr<ID3D11RenderTargetView> & pRenderTargetView, size_t bindId)
{
    bindId = bindId < RENDER_TARGET_MAX ? bindId : RENDER_TARGET_MAX;
    if (!bindId && pRenderTargetView == nullptr) { m_renderTargetViews[0] = m_myRenderTargetView.Get(); }
    else { m_renderTargetViews[bindId] = pRenderTargetView.Get(); }
}

void Renderer::ApplyRenderTargetView(size_t count)
{
    m_renderTargetViewCount = count < RENDER_TARGET_MAX ? count : RENDER_TARGET_MAX;
    m_deviceContext->OMSetRenderTargets(m_renderTargetViewCount, m_renderTargetViews, m_depthStencilView.Get());
}

void Renderer::SetDepthStencilView(const ComPtr<ID3D11DepthStencilView> & pDepthStencilview)
{
    if (pDepthStencilview == nullptr) { m_depthStencilView = m_myDepthStencilView; }
    else { m_depthStencilView = pDepthStencilview; }
}
...
void Renderer::CreateDepthStencilView()
{
    ...
    result = m_device->CreateDepthStencilView(m_depthStencilBuffer.Get(), &depthStencilViewDesc, &m_myDepthStencilView);
    if (FAILED(result))
        THROW_EX_HRESULT(InitializeRendererException, result);
}

// folyt kov
ComPtr<ID3D11DepthStencilState> Renderer::CreateStencilState(bool isEnable) const
{
    HRESULT result;
    ...
    result = m_device->CreateDepthStencilState(&dsDesc, pDSState.GetAddressOf());
...
void Renderer::SaveScreenshot(const std::string& filename) const
{
    ...
    if (pSurface)
    {
        ...
        if (pNewTexture)
        {
            ...
            int res = stbi_write_png(filename.c_str(), width, height, 4, captureData, pitch);
