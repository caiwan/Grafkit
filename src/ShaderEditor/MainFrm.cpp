...
void CMainFrame::UnregCmdMsg(CBasePane* pane){
	m_regCmdMsg.RemoveAt(m_regCmdMsg.Find(pane));
}
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	//m_wndLogger.EnableDocking(CBRS_ALIGN_ANY);
	//DockPane(&m_wndLogger);
...
BOOL CMainFrame::CreateDockingWindows()
{
	...
	//CString strLoggerWnd;
	//bNameValid = strLoggerWnd.LoadString(IDS_LOGGER_WND);
	//ASSERT(bNameValid);
	//if (!m_wndLogger.Create(strLoggerWnd, this, CRect(0, 0, 100, 100), TRUE, ID_VIEW_LOGGERWND, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_BOTTOM | CBRS_FLOAT_MULTI))
	//{
	...
	// Create shader editor window
	CString strShaderEditor;
#if 1
	bNameValid = strShaderEditor.LoadString(IDS_SHADER_EDITOR_WND);
	ASSERT(bNameValid);
#else
	strShaderEditor = "Shader editor";
...
void CMainFrame::SetDockingWindowIcons(BOOL bHiColorIcons)
{
	...
	//HICON hOutputBarIcon = (HICON) ::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(bHiColorIcons ? IDI_LOGGER_WND_HC : IDI_LOGGER_WND), IMAGE_ICON, ::GetSystemMetrics(SM_CXSMICON), ::GetSystemMetrics(SM_CYSMICON), 0);
	//m_wndLogger.SetIcon(hOutputBarIcon, FALSE);
