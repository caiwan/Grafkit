...
namespace {
	static const char pPSahderSrc[] = ""
		...
		"		if (udBox(uv, float2(1., THICK + BORDER))<LIM && udBox(uv, float2(1. - BORDER, THICK - BORDER))>LIM) return borderColor;\n"
		"		if (udBox(uv + float2(1. - transition, 0.), float2(transition - BORDER*2., THICK - BORDER*3.))<LIM) return barColor;\n"
		"		return float4(0.,0.,0.,0.);\n"
		"	}\n"
		"\n"
		"	float4 main(PixelInputType input) : SV_TARGET\n"
		"	{\n"
		"		float ar = barParams.x / barParams.y;\n"
		"	float2 uv = 2.*(input.tex.xy / barParams.y) - float2(ar, 1.);\n"
		"\n"
		"	float4 color = getLoader(uv * (1.8 / float2(ar, 1.)), barParams.z);\n"
		"	color.a = 1.;"
		"	return color;\n"
		"	}\n"
		;
