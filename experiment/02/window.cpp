...
bool Window::isMultisampleSupported = false;
...
void Window::createWindow(int sx, int sy, bool isfullscreen){
	this->w_width = sx;
	this->w_height= sy;
...
namespace {
	...
	LRWAPI window_callback_proc (HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
	{
		switch (msg) {
		case WM_DESTROY:
			PostQuitMessage(0);		// exit on window close
			return 0;
			break;
		case WM_KEYDOWN:
			keys[wp] = true;		// tablazat: http://delphi.about.com/od/objectpascalide/l/blvkc.htm
			if (wp == VK_ESCAPE)	PostQuitMessage(0);	// exit on ESC
														// DEV: more contol options are able to implement here.
			break;
		case WM_KEYUP:
			keys[wp] = false;
		}
